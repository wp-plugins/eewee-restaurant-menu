<?php
if( !class_exists(Form_DeviseAdd)){
	class Form_DeviseAdd extends WP_Query{
		 ...
		 * retourn form
 		 * @param $_POST $p
 		 */ 
		public function deviseAdd( $p="" ){ ?>
			<form method="post" action="<?php echo $this->_action; ?>">
				<input type='hidden' name='form_id' value='<?php echo $p[0]->ID_DEVISE; ?>' />
						...
						<p>
							<?php _e('Name', PLUGIN_NOM_LANG); ?> : <input type="text" name="form_nom" value="<?php echo $p[0]->NOM; ?>" />
                                                ...
                                                <p>
                                                        <?php
                                                        $selected0 = $selected1 = "";
                                                        if( $p[0]->LOCATION == 0 ){ $selected0 = "selected"; }
                                                        if( $p[0]->LOCATION == 1 ){ $selected1 = "selected"; }
                                                        ?>
							...
							<?php _e('Separator', PLUGIN_NOM_LANG); ?> : 
                                                        <select name='form_separator'>
                                                            ...
                                                            foreach( $separator as $k=>$v ){ 
                                                                if( $p[0]->SEPARATOR == $k ){ $selected = "selected"; }else{ $selected = ""; }
                                                                ?>

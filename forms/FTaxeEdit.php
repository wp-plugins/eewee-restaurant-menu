<?php
if( !class_exists(Form_TaxeEdit)){
	class Form_TaxeEdit extends WP_Query{
		...
		function __construct(){
			$this->_action      = $_SERVER["REQUEST_URI"];
                        $this->_returnUrl   = EEWEE_RESTAURANT_MENU_URL_BACK_SOUS_MENU_7;
		}
		 ...
		 * retourn form
 		 * @param array $r
 		 */ 
		public function taxeEdit( $r ){ ?>
			
			<form method="post" action="<?php echo $this->_action; ?>">
				<input type='hidden' name='form_id' value='<?php echo $r[0]->ID_TAXE; ?>' />

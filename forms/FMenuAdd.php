<?php
if( !class_exists(Form_menuAdd)){
	class Form_menuAdd extends WP_Query{
		 ...
		 * retourn form
 		 * @param $_POST $p
 		 */ 
		public function menuAdd( $p="" ){ ?>
			<form method="post" action="<?php echo $this->_action; ?>">
				<input type='hidden' name='form_id' value='<?php echo $p[0]->ID_menu; ?>' />
					...
					<div class="inside">
						...
						<p>
							<?php _e('Name', PLUGIN_NOM_LANG); ?> : <input type="text" name="form_nom" value="<?php echo $p[0]->NOM; ?>" />
                                                ...
                                                <p>
							<?php _e('Price', PLUGIN_NOM_LANG); ?> : <input type="text" name="form_prix" value="<?php echo $p[0]->PRIX; ?>" />
						</p>
						<p>
							<?php 
							$etat_on = $etat_off = "";
